...
app.controller('Analytics', ['$scope', '$http', function($scope, $http) {
	...
	var programaSelec = document.getElementById("programa").value;
	var programa;
	var tipoPrograma;

	spinner_up();

	//Default: MC 2016-2017
	if(!start_date && !end_date){
		start_date = '2016-07-01'; //Year-month-day
		var today = new Date();
	...
	if(programaSelec == '1'){
		programa = '1';
		tipoPrograma = 'person';
	}else if(programaSelec == '2'){
		programa = '2';
		tipoPrograma = 'person';
	}else if(programaSelec == '3'){
		programa = '1';
		tipoPrograma = 'opportunity';
	}else{
		programa = '2';
		tipoPrograma = 'opportunity';
	}
		...
		if (mcs !== null){
			for (var j = 0; j <= mcs.length - 1; j++) {
				options_rank_ogcdp = {
					...
					+ '&programmes%5B%5D=' + programa
					+ '&basic%5Btype%5D=' + tipoPrograma
	...
	function rank (ranking) {
	    ...
	    $('#container').highcharts({
	        ...
	        title: {
	            text: 'Rank<br>' + 'Fechas: ' + start_date + ' - ' + end_date
	        },
	        subtitle: {
